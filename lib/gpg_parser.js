...
GPGPackageParser.prototype = {
 ...
 _parse_clearsigned: function(packet){
    ...
    if(decoded) {
      ...
      throw "Base64 decoding failed, the string must be a cleartext-signature or an armored public key.";
      return null;
    }
    },
 ...
 get_signature_hash: function(){
    ...
    var sig_hash;
    switch(this.signature.signature_hash_algo) {
			case 1:
			case "MD5":
    ...
    if (parseInt(hash_control_bytes, 16) == parseInt(sig_hash.slice(0,4), 16)){
      ...
      return sig_hash;
			...
			this.log("Hash of the cleartext is wrong, 4 control-bytes did not match, " + hash_control_bytes + " != " + sig_hash.slice(0,4));
      return null;
    }
  },
...
$gp.verifyAuthentication = function(publickey, signature) {
  if(publickey.short_id != signature.issuer_id) throw "This signature was made with another key.";
...
GPGPackageParser.prototype.dsa = function(publickey, signature) {
  ...
  signature_hash = this.hexstring2bigInt(signature.hash.substr(0, Math.ceil(bitSize(q) / 4))); 
