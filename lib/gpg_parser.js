...
//	content = content.slice(start_content, end_content + 1).replace(/\n/gi, String.fromCharCode(13,10));//slice(start_content, end_content + 1);
	...
	if(this.signature.signature_hash_algo == 8) sig_hash = pidCrypt.SHA256(val_to_hash);
	    ...
	    sig_hash = "";
	    this.log("unknown hash algorithm, algo id: " + this.signature.signature_hash_algo);
	}
	if (this.signature.first16bit_hash.toString(16) == sig_hash.slice(0,4))
	    return sig_hash;
	else {
	    this.log("get_signature_hash failed, 4 control-bytes did not match");
	    return null;
	}
    },
...
Symcrypt.verifyAuthentication = function(publickey, signature) {
        ...
        if(signature.signature.signature_algo == "dsa") {
                ...
                if(signature.signature.signature_algo == "rsa") {
                        ...
                        this.log("no suitable authentication method provided");
                        return false;
                }
    }
...
Symcrypt.dsa = function(publickey, signature) {
        ...
        signature_hash = signature.hash.toUpperCase();
...
Symcrypt.rsa = function(publickey, signature) {
    ...
    //this.log("signature hash decrypted (hex): ");
    //this.log(bigInt2str(signature_hash_decrypted, 16));
