...
*                                                             *
**************************************************************/
var Symcrypt = Symcrypt || {};

Symcrypt.$gp = function(packet){
...
//	content = content.slice(start_content, end_content + 1).replace(/\n/gi, String.fromCharCode(13,10));//slice(start_content, end_content + 1);
	...
	if(this._next_packet_offset){
	    ...
	    return Symcrypt.$gp($.extend([], new_packet));
	}
	else return null;
    },
	...
	retval.issuer = Symcrypt.reduce(function(a,b){
	...
	if(this.signature.signature_hash_algo == 8) sig_hash = pidCrypt.SHA256(val_to_hash);
		...
		this.signature.signature_hash_algo == "SHA256") sig_hash = pidCrypt.SHA512(val_to_hash);
...
//     $.ajax({url: Symcrypt.extDir + "/gpgserver.cgi?service=getKey" +
// 	    "&id=" + "0xA0C33749",//"0x5677134A",//"0x05D578E9",//
// 	    method:"get",
// 	    dataType: "jsonp",
// 	    success:function(r){
// 		console.log(Symcrypt.$gp(r).get_keys());//.baToBInt([1,1,0x80,0,44], 1 , 3));
// 		//alert(r);//$.base64Decode(r));
// 	    }});
// });


Symcrypt.map = function(m,f) {
    var newm = [];
    for (i in m) {
        newm.push(f(m[i]));
    }
...
Symcrypt.verifyAuthentication = function(publickey, signature) {

        if(signature.signature.signature_algo == "dsa") {
                return Symcrypt.dsa(publickey, signature);
        }
        else {
                if(signature.signature.signature_algo == "rsa") {
                        return Symcrypt.rsa(publickey, signature);
                }
                else {
...
Symcrypt.dsa = function(publickey, signature) {
        ...
        signature_hash = signature.get_signature_hash().toUpperCase();
        ...
        if(!negative(r) && !isZero(r) && greater(q,r) && !negative(s) && !isZero(s) && greater(q,s)) {
                ...
                console.log("0 < r < q and 0 < s < q is not satisfied");
...
Symcrypt.rsa = function(publickey, signature) {
    //   publickey.get_keys();
